...
export default async function handler(req) {
      ...
      if (teams.length > 0) {
        ...
        teams.forEach(team => {
          ...
          if (reality.torneoIniciado) {
            extra = teamInPhase(team, ph.key, reality) ? ' chip-hit' : ' chip-miss';
          }
          chips += '<span class="chip' + extra + '">' + flag(team) + ' ' + team + '</span>';
        });
        bracketHtml +=
  ...
  const css =
    ':root{--black:#000000;--white:#FFFFFF;--lime:#C9FF24;--magenta:#FF0055;--teal:#00FFCC;--purple:#6200EA;--dim:#1F1F1F}' +
    '*{box-sizing:border-box;margin:0;padding:0}' +
    "body{background-color:var(--black);color:var(--white);font-family:'Inter',sans-serif;min-height:100vh;display:flex;justify-content:center}" +
    '.app-container{width:100%;max-width:450px;padding:0 16px 120px;margin:0 auto}' +
    "h1,.display-font{font-family:'Archivo Black',sans-serif;text-transform:uppercase;line-height:0.9}" +
    '.header-box{margin-top:40px;margin-bottom:30px;border-bottom:4px solid var(--white);padding-bottom:10px}' +
    '.badge-26{display:inline-block;background:var(--white);color:var(--black);font-weight:900;font-size:14px;padding:4px 8px;margin-bottom:12px;letter-spacing:-0.5px}' +
    ...
    ".section-title{font-family:'Archivo Black';font-size:24px;margin-bottom:16px;letter-spacing:-0.5px;border-left:8px solid var(--magenta);padding-left:12px}" +
  ...
  const html = '<!DOCTYPE html><html lang="es"><head>' +
    ...
    '<div><div class="lbl">' + t.exacts + '</div><div class="val">' + (profile.pronosticos_correctos || 0) + '</div></div>' +
    '</div>' +
    '<div class="points-detail">' +
    '<div class="pd-box"><div class="lbl">' + t.groups + '</div><div class="val">' + (profile.puntos_goles || 0) + '</div></div>' +
    '<div class="pd-box secondary"><div class="lbl">' + t.brackets + '</div><div class="val">' + (profile.puntos_brackets || 0) + '</div></div>' +
    '</div>' +
    '</div>' +
